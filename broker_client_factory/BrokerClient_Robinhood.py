...
from IBridgePy.IbridgepyTools import symbol, from_string_to_security
from IBridgePy.constants import LiveBacktest, OrderStatus, OrderType, BrokerName, OrderTif, OrderAction
from IBridgePy.quantopian import from_security_to_contract, from_contract_to_security
from broker_client_factory.BrokerClientDefs import ReqAttr, ReqPositions, ReqAccountUpdates, ReqAllOpenOrders, \
    ReqMktData
...
class BrokerClientRobinhood(CallBacks):
    ...
    def setup_brokerClient_Robinhood(self, log, rootFolderPath, singleTrader, dataFromServer, timeGenerator,
        ...
        self.log.debug(__name__ + '::setup_brokerClient_Robinhood')
    ...
    def disconnectWrapper(self):
        self.log.debug(__name__ + '::disconnectWrapper')
    ...
    def reqPositionsWrapper(self):
        ...
        for position in ans:
            ...
            contract = from_security_to_contract(security)
            self.simulatePosition(self.accountCode,
                                  contract,
    ...
    def _convert_order(self, rbOrder):
        int_orderId, contract, ibOrder, orderState, security = OrderConverter().fromRBtoIBOpenOrder(rbOrder, self._idConverter, self.accountCode)
        # IBCpp function
    ...
    def reqAccountUpdatesWrapper(self, subscribe, accountCode):
        ...
        if accountCode != correctAccountCode:
            self.log.error(__name__ + '::reqAccountUpdatesWrapper: EXIT, input accountCode=%s is not correct, '
                                      'it should be %s' % (accountCode, correctAccountCode))
            exit()
    ...
    def reqHistoricalDataWrapper(self, reqId, contract, endTime, goBack, barSize, whatToShow, useRTH, formatDate):
        if barSize not in ['1 day', '5 mins', '10 mins']:
            self.log.error(__name__ + '::reqHistoricalDataWrapper: EXIT, Robinhood does not support barSize=%s'
                           % (barSize,))
        interval = BarSizeConverter().fromIBtoRB(barSize)
        span = GoBackConverter().fromIBtoRB(goBack)
        self.log.notset(__name__ + '::reqHistoricalDataWrapper: contract=%s interval=%s span=%s'
        ...
        try:
            ...
            self.log.error(__name__ + '::reqHistoricalDataWrapper: EXIT, ans=%s' % (ans,))
            exit()

        for row in hist:
            t = dt.datetime.strptime(row['begins_at'], "%Y-%m-%dT%H:%M:%SZ")
            epoc = dt_to_utc_in_seconds(t)
            if formatDate == 2:
                date = epoc
            else:
                date = utc_in_seconds_to_dt(float(epoc))
    ...
    def placeOrderWrapper(self, contract, order, ibpyRequest):
        ...
        if order.orderType == OrderType.MKT:
            ...
            self.log.error(__name__ + '::placeOrderWrapper: EXIT, cannot handle orderType=%s' % (order.orderType,))
            exit()

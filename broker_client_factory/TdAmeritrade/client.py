...
class TDClient(object):
    ...
    def _is_access_token_valid(self):
        ...
        if (dt.date.today() - self._refresh_token_createdOn) >= dt.timedelta(80):
            self.log.error(
                'TD refresh token is about to expire on %s' % (self._refresh_token_createdOn + dt.timedelta(days=90),))
            self.log.error('Hint: Refer to Youtube video to get a new refresh token https://youtu.be/aT1nB-vMqdE')
    ...
    def _renew_access_token(self):
        if not self._is_access_token_valid():
            refresh = auth.refresh_token(self._refresh_token, self._apiKey)
    ...
    def place_orders(self, account_id, order):
        ...
        if response.status_code == 201:
            ...
            self.log.error(__name__ + '::place_orders: EXIT, Place order failed. response=%s.\n'
                                      'Suggestion:\n'
    ...
    def cancel_order(self, account_id, order_id):
        ...
        if response.status_code == 200:
            ...
            self.log.error(__name__ + '::cancel_order: EXIT, cancel_order failed. response=%s' % (response.content,))
            exit()

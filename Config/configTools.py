...
def _get_user_input(globalsV):
    ...
    for item in ['logLevel', 'runMode', 'dataProviderName', 'marketName', 'timeGeneratorType', 'showTimeZone']:
        if item in globalsV:
            if item == 'logLevel':
                setattr(user_input, item, LogLevel().get(globalsV[item]))
            if item == 'runMode':
                setattr(user_input, item, TraderRunMode().get(globalsV[item]))
            if item == 'dataProviderName':
                setattr(user_input, item, DataProviderName().get(globalsV[item]))
            if item == 'marketName':
                setattr(user_input, item, MarketName().get(globalsV[item]))
            if item == 'timeGeneratorType':
                setattr(user_input, item, TimeGeneratorType().get(globalsV[item]))

...
def search_security_in_file(df, secType, ticker, currency, param):
    ...
    if tmp_df.shape[0] == 1:  # found 1
        ...
        print(__name__ + r'::search_security_in_file: EXIT (%s %s %s) must be a unique combination in IBridgePy/security_info.csv' % (secType, ticker, currency))
        exit()
    else:  # found None
...
def convert_market_hours_to_df(hoursString):
    ...
    d = pd.DataFrame(v, columns=['date', 'open', 'close'])
    d = d.set_index('date')
    return d

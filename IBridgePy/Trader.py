...
from IBridgePy.quantopian import TimeBasedRules, calendars, MarketOrder, from_contract_to_security
...
class Trader(object):
    ...
    def __init__(self):
        ...
        self.sysTimeZone = None
        self.showTimeZone = None
        self.runScheduledFunctionBeforeHandleData = None
    ...
    def update_from_userConfig(self, userConfig):
        self.fileName = userConfig.projectConfig.fileName
        ...
        self.sysTimeZone = userConfig.projectConfig.sysTimeZone

        self.showTimeZone = userConfig.projectConfig.showTimeZone
        self.log = userConfig.log
        self.userLog = userConfig.userLog
        ...
        self.runScheduledFunctionBeforeHandleData = userConfig.projectConfig.runScheduledFunctionBeforeHandleData
        ...
        if isinstance(self.accountCode, list) or isinstance(self.accountCode, set) or isinstance(self.accountCode,
            ...
            self.accountCodeSet = {self.accountCode}  # change from string to set

        # set up context and qData.data
        self.context = Context(self, self.accountCodeSet)
        self.qData = QDataClass(self)
        if self.accountCode == '':
            print(__name__ + '::build: EXIT, do not forget to change accountCode in userConfig')
            exit()
    ...
    def get_next_time(self):
        timeNow = self.brokerService.get_next_time().astimezone(self.sysTimeZone)
        self.log.notset(__name__ + '::get_next_time: timeNow=%s' % (timeNow,))
        return timeNow
    ...
    def get_option_greeks(self, security, tickType, fields):
        ...
        return self.brokerService.get_option_greeks(security, self.tick_type_mapper[tickType], fields)
    ...
    def processMessages(self, timeNow):
        ...
        :return:
        """
        self.log.notset(__name__ + '::process_messages: timeNow=%s' % (str(timeNow),))
    ...
    def get_datetime(self, timezone='default'):
        ...
        if timezone == 'default':
            ans = timeNow.astimezone(self.showTimeZone)
        else:
            ans = timeNow.astimezone(pytz.timezone(timezone))
        self.log.notset(__name__ + '::get_datetime: timeNow=%s' % (timeNow,))
        return ans
    ...
    def initialize_Function(self):
        ...
        if self.multiAccountFlag:
            self.brokerService.submit_requests(
                ReqAccountSummary(),
                ReqAllOpenOrders(),
                ReqPositions())
        else:
    ...
    def repeat_Function(self, dummyTime):
        self.log.notset(__name__ + '::repeat_Function: dummyTime=%s' % (str(dummyTime),))
    ...
    def display_orderStatus(self, accountCode='default'):
        ...
        if len(orders) >= 1:
            ...
            for ibpyOrderId in orders:
                self.log.info(str(orders[ibpyOrderId]))
            self.log.info('##    END    ##')
        else:
    ...
    def display_account_info(self, accountCode='default'):
        ...
        adj_accountCode = self.adjust_accountCode(accountCode)
        self.log.info('##    ACCOUNT Balance  %s  ##' % (adj_accountCode,))
        a = self.brokerService.get_account_info(adj_accountCode, 'TotalCashValue')
        b = self.brokerService.get_account_info(adj_accountCode, 'NetLiquidation')
        c = self.brokerService.get_account_info(adj_accountCode, 'GrossPositionValue')
        # d = self.brokerService.get_account_info(adj_accountCode, 'AvailableFunds')  # backtester is not ready 20200819
        self.log.info('CASH=%s' % (a,))
        self.log.info('portfolio_value=%s' % (b,))
        self.log.info('positions_value=%s' % (c,))
        # self.log.info('available_funds=%s' % (d,))
    ...
    def display_all(self, accountCode='default'):
        self.log.debug(__name__ + '::display_all: accountCode=%s' % (accountCode,))
    ...
    def _get_portfolio(self, accountCode):
        self.log.notset(__name__ + '::_get_portfolio: accountCode=%s' % (accountCode,))
        adj_accountCode = self.adjust_accountCode(accountCode)
    ...
    def show_account_info(self, field, meta='value', accountCode='default'):
        ...
        """
        self.log.debug(__name__ + '::show_account_info: field=%s accountCode=%s' % (field, accountCode))
        adj_accountCode = self.adjust_accountCode(accountCode)
        ans = self.brokerService.get_account_info(adj_accountCode, field, meta)
        if ans:
            return ans
        self.log.error(__name__ + '::show_account_info: EXIT, field=%s is not accessible accountCode=%s' % (field, accountCode))
        exit()

    def hold_any_position(self, accountCode='default'):
        self.log.debug(__name__ + '::hold_any_position')
        adj_accountCode = self.adjust_accountCode(accountCode)
        return len(self.brokerService.get_all_positions(adj_accountCode)) != 0
    ...
    def show_real_time_price(self, security, param):
        self.log.notset(__name__ + '::show_real_time_price: security=%s param=%s' % (security, param))
        # self.validator.showRealTimePriceValidator.validate(security, adj_param)
        self.brokerService.add_exchange_primaryExchange_to_security(security)
        return self.brokerService.get_real_time_price(security, self.tick_type_mapper[param])
    ...
    def show_timestamp(self, security, param):
        ...
        """
        self.log.notset(__name__ + '::show_timestamp: security=%s param=%s' % (security, param))
        self.brokerService.add_exchange_primaryExchange_to_security(security)
        return self.brokerService.get_timestamp(security, self.tick_type_mapper[param])
    ...
    def show_real_time_size(self, security, param):
        ...
        self.brokerService.add_exchange_primaryExchange_to_security(security)
        return self.brokerService.get_real_time_size(security, self.tick_type_mapper[param])
    ...
    def cancel_order(self, ibpyOrderId):
        ...
        if isinstance(ibpyOrderId, str):
            ...
            self.brokerService.cancel_order(ibpyOrderId)
        elif isinstance(ibpyOrderId, int):
            self.log.error(__name__ + '::cancel_order: ibpyOrderId must be a string')
            exit()
        elif isinstance(ibpyOrderId, IbridgePyOrder):
            self.log.debug(__name__ + '::cancel_order: ibpyOrderId=%s' % (ibpyOrderId.getIbpyOrderId(),))
            self.brokerService.cancel_order(ibpyOrderId.getIbpyOrderId())
    ...
    def create_order(self, action, amount, security, orderDetails, ocaGroup=None, ocaType=None, transmit=None,
        ...
        int_orderId = self.brokerService.use_next_id()
        adj_accountCode = self.adjust_accountCode(accountCode)
        if action == 'BUY':
            amount = abs(amount)
        else:
    ...
    def order(self, security, amount, style=MarketOrder(), orderRef='',
        ...
        self.log.debug(__name__ + '''::order: security=%s amount=%s style=%s orderRef=%s accountCode=%s outsideRth=%s hidden=%s''' % (security.full_print(), amount, style, orderRef, accountCode, outsideRth, hidden))
        adj_accountCode = self.adjust_accountCode(accountCode)
        int_orderId = self.brokerService.use_next_id()
        self.brokerService.add_exchange_primaryExchange_to_security(security)
        ...
        return self.brokerService.place_order(ibridgePyOrder, followUp=followUp,
    ...
    def order_percent(self, security, percent, style=MarketOrder(), orderRef='', followUp=True,
        ...
        if percent > 1.0 or percent < -1.0:
            ...
            exit()
        portfolio_value = self.brokerService.get_account_info(adj_accountCode, 'NetLiquidation')
    ...
    def order_target(self, security, amount, style=MarketOrder(), orderRef='', followUp=True,
                     accountCode='default', outsideRth=False):
        self.log.debug(__name__ + '::order_target: security=%s amount=%s' % (security.full_print(), amount))
        adj_accountCode = self.adjust_accountCode(accountCode)
        ...
        if amount != hold:
            ...
            return self.order(security, amount=int(amount - hold), style=style,
                              orderRef=orderRef, accountCode=adj_accountCode, followUp=followUp, outsideRth=outsideRth)
        else:
    ...
    def order_target_percent(self, security, percent, style=MarketOrder(), orderRef='', followUp=True,
                             accountCode='default',outsideRth=False):
        self.log.notset(__name__ + '::order_target_percent')
        adj_accountCode = self.adjust_accountCode(accountCode)
        if percent > 1.0 or percent < -1.0:
            self.log.error(__name__ + '::order_target_percent: EXIT, percent=%s [-1.0, 1.0]' % (str(percent),))
            exit()
        a = self.brokerService.get_account_info(adj_accountCode, 'NetLiquidation')
        b = self.show_real_time_price(security, 'ask_price')
        if math.isnan(b):
            self.log.error(__name__ + '::order_target_percent: EXIT, No real time price is available right now. Market closed? real_time_price is NaN security=%s' % (security.full_print(),))
            exit()
        if b <= 0.0:
            self.log.error(__name__ + '::order_target_percent: EXIT, No real time price is available right now. Market closed? real_time_price=%s security=%s' % (b, security.full_print()))
            exit()
    ...
    def place_order_with_stoploss(self, security, amount, stopLossPrice, style=MarketOrder(), tif='DAY',
        ...
        str_parentOrderId = self.brokerService.place_order(parentOrder, followUp=False)
        str_slOrderId = self.brokerService.place_order(slOrder)
    ...
    def place_order_with_takeprofit(self, security, amount, takeProfitPrice, style=MarketOrder(), tif='DAY',
        ...
        # As IB recommended, parentOrder and takeProfitOrder are not transmitted, so that IBridgePy should not follow up
        str_parentOrderId = self.brokerService.place_order(parentOrder, followUp=False)
        str_tpOrderId = self.brokerService.place_order(tpOrder)
    ...
    def place_order_with_stoploss_takeprofit(self, security, amount, stopLossPrice, takeProfitPrice,
        ...
        parentOrder = create_order(int_parentOrderId, adj_accountCode, security, amount, style, self.get_datetime(),
                                   ocaGroup=ocaGroup)
        int_tpOrderId = self.brokerService.use_next_id()
        tpOrder = create_order(int_tpOrderId, adj_accountCode, security, -amount, LimitOrder(takeProfitPrice, tif=tif),
                               self.get_datetime(),
                               ocaGroup=ocaGroup)
        int_slOrderId = self.brokerService.use_next_id()
        ...
        # As IB recommended, parentOrder and takeProfitOrder are not transmitted, so that IBridgePy should not follow up
        str_parentOrderId = self.brokerService.place_order(parentOrder, followUp=False)
        str_tpOrderId = self.brokerService.place_order(tpOrder, followUp=False)
        str_slOrderId = self.brokerService.place_order(slOrder)
    ...
    def place_combination_orders(self, legList):
        ...
        for order in legList:
            ibpyOrderId = self.brokerService.place_order(order, followUp=False)
    ...
    def modify_order(self, ibpyOrderId, newQuantity=None, newLimitPrice=None, newStopPrice=None, newTif=None,
        ...
        self.log.debug(__name__ + '::modify_order: ibpyOrderId = %s' % (ibpyOrderId,))
        currentOrder = self.brokerService.get_order(ibpyOrderId)
        if currentOrder.status in [OrderStatus.PRESUBMITTED, OrderStatus.SUBMITTED]:
            ...
            self.brokerService.submit_requests(ModifyOrder(ibpyOrderId, currentOrder.contract, currentOrder.order,
                                                           FollowUpRequest.DO_NOT_FOLLOW_UP))
        else:
            self.log.error(__name__ + '::modify_order: Cannot modify ibpyOrderId=%s orderStatus=%s' % (ibpyOrderId, currentOrder.status))
            exit()
    ...
    def get_order_status(self, ibpyOrderId):
        ...
        """
        return self.brokerService.get_order(ibpyOrderId).status
    ...
    def order_status_monitor(self, ibpyOrderId, target_status, waitingTimeInSeconds=30):
        ...
        does not specifically check the order status.
        order_status_monitor is used to follow up on the order status.
        ...
        """
        self.log.notset(
            __name__ + '::order_status_monitor: ibpyOrderId=%s target_status=%s' % (ibpyOrderId, target_status))
        if not isinstance(ibpyOrderId, str):
            self.log.error(__name__ + '::order_status_monitor: EXIT, ibpyOrderId=%s must be a string' % (ibpyOrderId,))
            exit()
        if ibpyOrderId == 'NoOrderPlaced':
            return True
        return self.brokerService.order_status_monitor(ibpyOrderId, target_status, waitingTimeInSeconds)

    def get_all_open_orders(self, accountCode='default'):
        adj_accountCode = self.adjust_accountCode(accountCode)
        return self.brokerService.get_all_open_orders(adj_accountCode)
    ...
    def get_order(self, ibpyOrderId):
        ...
        """
        self.log.debug(__name__ + '::get_order: ibpyOrderId=%s' % (ibpyOrderId,))
        return self.brokerService.get_order(ibpyOrderId)
    ...
    def get_open_orders(self, security=None, accountCode='default'):
        ...
        adj_accountCode = self.adjust_accountCode(accountCode)
        orderList = self.brokerService.get_all_open_orders(adj_accountCode)
        if security is None:
            ans = {}
    ...
    def get_all_positions(self, accountCode='default'):
        ...
        adj_accountCode = self.adjust_accountCode(accountCode)
        return self.brokerService.get_all_positions(adj_accountCode)
    ...
    def get_position(self, security, accountCode='default'):
        ...
        """
        self.log.debug(__name__ + '::get_position: security=%s' % (security.full_print(),))
        adj_accountCode = self.adjust_accountCode(accountCode)
    ...
    def _check_schedules(self):
        ...
        self.log.notset(__name__ + '::_check_schedules: timeNow=%s' % (timeNow,))
    ...
    def calculate_cash_for_trade_deprecated(self, accountCode='default'):
        ...
        """
        self.log.debug(__name__ + '::calculate_cash_for_trade_deprecated:')
        adj_accountCode = self.adjust_accountCode(accountCode)
        ...
        for ibpyOrderId in orderDict:
            ...
            if order.status in [OrderStatus.SUBMITTED, OrderStatus.PRESUBMITTED]:
                ...
                if orderType == OrderType.LMT:
                    ...
                    cashOccupied += order.limit * order.amount
                    cashOccupied += order.stop * order.amount
        return self.brokerService.get_account_info(adj_accountCode, 'AvailableFunds') - cashOccupied

    # noinspection PyTypeChecker
    def close_all_positions_LMT(self, ls_excludeSecurities, advantage, orderStatusMonitor=True, accountCode='default'):
        self.log.debug(__name__ + '::close_all_positions_LMT:')
        # if advantage < 0.0:
        #     self.log.error(__name__ + '::close_all_positions_LMT: EXIT, advantage=%s must > 0.0' % (advantage,))
        #     exit()
        adj_accountCode = self.adjust_accountCode(accountCode)
        positions = self.get_all_positions(adj_accountCode)  # return a dictionary
        orderIdList = []
        ...
        for security in adj_securities:
            if not _is_in_security_list(security, ls_excludeSecurities):
                ...
                if holding_quantity > 0:
                    ...
                    self.log.info(__name__ + '::close_all_positions_LMT: bid_price=%s, sellAtLimitPrice=%s' % (bid_price, limitPrice))
                    ...
                    self.log.info(__name__ + '::close_all_positions_LMT: ask_price=%s, buyAtLimitPrice=%s' % (ask_price, limitPrice))
                    ibpyOrderId = self.order(security, -holding_quantity,
                                             style=LimitOrder(limitPrice, tif=OrderTif.GTC),
                                             accountCode=adj_accountCode)
                orderIdList.append(ibpyOrderId)
    ...
    def rebalance_portfolio(self, traderDecisions, buyAdvantage=0.0, sellAdvantage=0.0, accountCode='default'):
        ...
        self.log.info('Start to sell securities not in the traderDecisions')
        ...
        self.log.info('Finished to sell securities not in the traderDecisions')
        ...
        for security in traderDecisions:
            ...
            if target_quantity < holding_quantity:
                ...
                self.log.debug('No action for security=%s' % (security,))

        self.log.info('sellSecurityDict=%s' % (print_decisions(sellSecurityDict),))
        self.log.info('buySecurityDict=%s' % (print_decisions(buySecurityDict),))
        ...
        while len(buySecurityDict) > 0 and (dt.datetime.now() - startTime).total_seconds() < 60 * 5:
            ...
            self.log.info(__name__ + '::rebalance_portfolio: available_fund=%s' % (available_fund,))
            for security in buySecurityDict:
                ...
                if purchase_amount > 0:
                    ...
                    self.log.error(__name__ + '::rebalance_portfolio: EXIT, purchase_amount=%s target_quantity=%s available_fund=%s ask_price=%s' % (purchase_amount, target_quantity, available_fund, ask_price))
                    exit()
        ...
        self.log.info(__name__ + '::rebalance_portfolio: completed')
    ...
    def record(self, *args, **kwargs):
        self.userLog.record(*args, **kwargs)

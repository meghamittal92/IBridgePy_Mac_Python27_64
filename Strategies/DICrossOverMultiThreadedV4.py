...
fileToStoreResults = "BackTestOutputs/" + '_'.join(['_'.join(securities), 'LiveV3', originalCandleTimeFrame.replace(" ", ""),str(ADX_EXIT_THRESHHOLD), str(ADX_ENTRY_THRESHHOLD) + '.txt'])


def initialize(context):
    context.run_once=False
    sample = open(fileToStoreResults, 'a', buffering=1)
    context.file = sample
    printToFile(context, "#######\nAT BEGINNING\n", printToScreen = True)
    context.file.write("######" + "\n")
    print("######" + "\n")
...
def placeOrder(security, targetPercent):
    ...
    print("Limit proce " + str(limitPrice))
...
def trade(context,data, dataExtendedTimeFrame):
    ...
    if(isPositiveCrossover(data)):
        if(dataExtendedTimeFrame['posDI'][-1] < dataExtendedTimeFrame['negDI'][-1] or (not isADXAboveEntryThreshhold(data)) or isAfterFourPM(sTime)):
            takeNewPos = False

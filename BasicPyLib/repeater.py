...
class Repeater:
    ...
    def schedule_event(self, event):
        if isinstance(event, Event.SpotTimeEvent):
            ...
            self.log.error(__name__ + '::schedule_event: Exit, cannot handle event=%s' % (event,))
            exit()
    ...
    def repeat(self):
        self.log.debug(__name__ + '::repeat: spotEvents=%s' % (self.spotEvents,))
        self.log.debug(__name__ + '::repeat: repeatedEvents=%s' % (self.repeatedEvents,))
        self.log.debug(__name__ + '::repeat: conceptEvents=%s' % (self.conceptEvents,))
        self.log.debug(__name__ + '::repeat: hftEvents=%s' % (self.hftEvents,))

        while True:
            try:
                self.log.notset(__name__ + '::repeat: ####    START a new datetime    ####')
                timeNow = self.getTimeNowFuncGlobal()
                self.log.notset(__name__ + '::repeat: timeNow=%s' % (timeNow,))
            except StopIteration:
                break

            if self.stopFuncGlobal(timeNow):
                self.log.notset(__name__ + '::repeat: timeNow=%s not go through repeat engine')
                break
